    ...
    public class RegisterValidator
    {
        public static AppError? Validate(RegisterRequest request)
        {
            if (string.IsNullOrWhiteSpace(request.FirstName) || string.IsNullOrWhiteSpace(request.LastName))
                return AppError.Validation(ErrorMessages.Get("Register:FirstNameAndLastNameRequired"));
            ...
            if (!Regex.IsMatch(request.Password, @"^(?=.*[A-Z])(?=.*[a-z])(?=.*\d)(?=.*[\W_]).+$"))
                return AppError.Validation(ErrorMessages.Get("Register:PasswordMustIncludeUpperLowerNumberSymbol"));
